...
class JosephrowntreeExtractor(BaseExtractor):
    ...
    def _scrape_show(self, driver, show: dict) -> dict | None:
        ...
        self.custom_logger.debug(f"DEBUG performances raw: {performances}")
        self.custom_logger.debug(f"DEBUG dates list: {dates}")
        # Convert ISO date strings to `date` objects for correct chronological comparisons
        date_objs = [date.fromisoformat(d) for d in dates]
        earliest_date_obj = min(date_objs)
        first_perf_date_obj = date.fromisoformat(performances[0]["date"]) if performances else earliest_date_obj
        open_date_obj = min(earliest_date_obj, first_perf_date_obj)
        close_date_obj = max(date_objs)
        ...
        # Defensive check: ensure open_date is not after the earliest upcoming performance.
        try:
            perf_earliest = min(date.fromisoformat(p["date"]) for p in performances)
            rec_open = date.fromisoformat(record["open_date"]) if record.get("open_date") else None
            if rec_open and rec_open > perf_earliest:
                record["open_date"] = perf_earliest.isoformat()
                record["booking_start_date"] = perf_earliest.isoformat()
        except Exception:
            pass
